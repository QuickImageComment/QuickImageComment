...
namespace Brain2CPU.ExifTool
{
    ...
    public sealed class ExifToolWrapper : IDisposable
    {
        ...
        public static void init(string iniPath, string language, string exifToolPath = null, bool faster = false)
        {
            ...
            Start();
            // first try to get locations from file (is faster)
            // if no file for this version exists, get locations from exiftool
            Locations.Clear();
            FillLocationListFromFile(iniPath);
            if (Locations.Count == 0)
            {
                FillLocationListFromExifTool(iniPath);
            }
        ...
        public static void FillLocationListFromExifTool(string iniPath)
        {
            ...
            if (cmdRes)
            {
                ...
                try
                {
                    StreamOut = new System.IO.StreamWriter(getLocationFileName(iniPath), false, System.Text.Encoding.UTF8);
                }
                catch (UnauthorizedAccessException)
                {
                    GeneralUtilities.message(LangCfg.Message.E_configFileNoWriteAccess, getLocationFileName(iniPath));
                    return;
                }
                StreamOut.WriteLine("; Locations from ExifTool " + ExifToolVersion);
                StreamOut.WriteLine("; Created by QuickImageComment - DO NOT EDIT THIS FILE!");
                StreamOut.WriteLine("; --------------------------------------------------------------------------");
                foreach (string word in Locations)
                {
                    StreamOut.WriteLine(word);
                }
                StreamOut.Close();
            }
        }

        public static void FillLocationListFromFile(string iniPath)
        {
            if (System.IO.File.Exists(getLocationFileName(iniPath)))
            {
                System.IO.StreamReader StreamIn =
                  new System.IO.StreamReader(getLocationFileName(iniPath), System.Text.Encoding.UTF8);
                string line = StreamIn.ReadLine();
                while (line != null)
                {
                    if (!line.StartsWith(";")) Locations.Add(line);
        ...
        private static string getLocationFileName(string iniPath)
        {
            return iniPath + "QIC-exifTool-" + ExifToolVersion + "-Locations.txt";
        }
                    ...
                    for (int ii = 1; ii < lines.Length; ii++)
                    {
                        ...
                        else if (lines[ii].StartsWith(" <tag id="))
                        {
                            ...
                            while (lines[ii + 1].StartsWith("  <desc lang="))
                            {
                                ...
                                if (words[2].Equals(language)) descriptionTranslated = words[3];
                                ii++;
                            }
