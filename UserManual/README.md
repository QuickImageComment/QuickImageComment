...
The Word document in output folder must be a trusted document. So it is recommended to add the output folder to trusted locations in MS Word.
...
Note: Before starting the batch files, close all instances of Words. If Word is already running, macros may be disabled, even if the folder is a trusted location.
